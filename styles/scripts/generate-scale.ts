...
function scaleUtility(name: string, properties: string | string[]) {
  ...
  const utility = `@utility coi-${name}-* {
	${propertiesArray
    .map(
      (property) =>
  ...
  const autoCompleteUtility = `@utility coi-${name}-px {
	${propertiesArray
...
function columnScaleUtility(name: string, properties: string | string[]) {
  const propertiesArray = Array.isArray(properties) ? properties : [properties]
  const utility = `@utility coi-${name}-col-* {
	${propertiesArray
    .map(
      (property) =>
  ...
  const autoCompleteUtility = `@utility coi-${name}-col-value {
	${propertiesArray
    .map(
      (property) =>
