...
${formatObject(
  ...
  ${formatObject(value as Record<string, unknown>, ([key, value]) => `--color-${key}: ${value};`)}
}`,
  '\n'
)}
  `
    ...
    .map(([key, value]) => {
      if (key === 'font-size') {
        ...
        if (
          ...
          return [
            `font-size: ${scalingCalc((value as { mobile: number; desktop: number }).mobile)};`,
            `@variant dt { font-size: ${scalingCalc((value as { mobile: number; desktop: number }).desktop)}; }`,
          ].join('\n\t')
        }
...
@utility coi-layout-block {
	margin-inline: auto;
  width: calc(100% - 2 * var(--safe));
}

@utility coi-layout-block-inner {
	padding-inline: var(--safe);
	width: 100%;
}

@utility coi-layout-grid {
	@apply coi-layout-block coi-grid;
}

@utility coi-layout-grid-inner {
	@apply coi-layout-block-inner coi-grid;
}`
